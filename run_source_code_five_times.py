...
def filter_line_profiler_results(line_profiler_output, top_n=5):
    lines = line_profiler_output.strip().split('\n')
    results = []

    for line in lines:
        match = re.search(r'(\d+\.\d+)\s+(\d+)\s+(.*)', line.strip())
        if match:
            time = float(match.group(1))
            line_no = int(match.group(2))
            code = match.group(3)
            results.append((time, line_no, code))

    total_time = sum(time for time, _, _ in results)
    results = [(time / total_time, line_no, code) for time, line_no, code in results]
    results.sort(reverse=True)

    top_lines = results[:top_n]
    output = "Line Profiler - Top lines by execution time:\n"
    for percentage, line_no, code in top_lines:
        output += f"Line {line_no}: {code.strip()} ({percentage:.2%})\n"

    return output.strip()

def filter_memory_profiler_results(memory_profiler_output, top_n=5):
    lines = memory_profiler_output.strip().split('\n')
    results = []

    for line in lines:
        match = re.search(r'(\d+\.\d+)\s+MiB\s+(\d+)\s+(.*)', line.strip())
        if match:
            memory = float(match.group(1))
            line_no = int(match.group(2))
            code = match.group(3)
            results.append((memory, line_no, code))

    results.sort(reverse=True)

    top_lines = results[:top_n]
    output = "Memory Profiler - Top lines by memory usage:\n"
    for memory, line_no, code in top_lines:
        output += f"Line {line_no}: {code.strip()} ({memory} MiB)\n"
...
"""
    ...
    try:
        ...
        line_profiler_results = filter_line_profiler_results(line_profiler_results)
        memory_report = filter_memory_profiler_results(memory_report)
        executable = True
        overhead = f"""
...
if __name__ == "__main__":
    ...
    for task in ["leetcode"]:
        for time in range(1,6):
            for model in models:
                if "/" in model:
                    model = model.split("/")[1]
                dat_path = f"./rebuttal/{task}_{model}_{time}_variation"

                try:
                    if model == "canonical_solution":
                        with open(f"./rebuttal/{task}_OpenCodeInterpreter-DS-1.3B.json", "r") as f:
                            dataset = json.load(f)
                    else:
                        with open(f"./rebuttal/{task}_{model}_5.json", "r") as f:
                            dataset = json.load(f)
                except Exception as e:
                    print(e)
                    continue
